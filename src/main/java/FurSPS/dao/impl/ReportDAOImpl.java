...
public class ReportDAOImpl implements IReportDAO {
	...
	public List<List<Object>> reportTotalMoneyInMonth() {
		String sql = "SELECT  " 
				...
				+ "    COUNT(OrderID) AS SL " 
				+ "FROM [ORDER] WHERE MONTH(OrderDate) = MONTH(CURDATE()) AND YEAR(OrderDate) = YEAR(CURDATE()) " + "GROUP BY Ngay " 
				+ "ORDER BY Ngay ";
	...
	public List<MyItem> reportBestItemSeller(int id) {
		List<MyItem> list = new ArrayList<>();
		String sql ="SELECT [PRODUCT].ProductID,[PRODUCT].ProductName, COUNT([ORDER].OrderID) AS OrderCount "
				+ "FROM [ORDER] "
				+ "JOIN [DETAIL] ON FurSPS.ORDER.OrderID = FurSPS.DETAIL.OrderID "
				+ "join [ITEM] on FurSPS.DETAIL.ItemID = FurSPS.ITEM.ItemID "
				+ "join [PRODUCT] on FurSPS.ITEM.ProductID = FurSPS.PRODUCT.ProductID "
				+ "WHERE [ORDER].SellerID = ? AND FurSPS.ORDER.Status=4 "
