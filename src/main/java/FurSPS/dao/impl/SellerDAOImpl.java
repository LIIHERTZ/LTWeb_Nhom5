...
public class SellerDAOImpl implements ISellerDAO {
	...
	public List<UserModel> findAllSeller() {
		...
		String sql = "Select UserID, FirstName, LastName, Address, Gender, Phone, DoB, CID, Avatar, KPI, Email from [USER] where Type=1";
	...
	public void insertSeller(UserModel model) {
		Connection conn = null;
		String sql = "Insert into [USER](UserID, FirstName, LastName, Address, Gender, Phone, DoB, CID, Avatar, KPI, Email, Type) Values (?,?,?,?,?,?,?,?,?,?,?,1)";
		try {
			conn = DBConnection.getConnection();// ket noi csdl
			PreparedStatement ps = conn.prepareStatement(sql);// nem cau lenh sql bang phat bieu prepare
			// gan gia tri tham so
	...
	public List<UserModel> findBestSeller() {
		Connection conn = null;
		String sql = "Select [USER].*, count(FurSPS.DETAIL.ItemID) as SL\r\n"
				+ "from [USER] \r\n"
				+ "join [ORDER] on FurSPS.USER.UserID = FurSPS.ORDER.SellerID\r\n"
				+ "join [DETAIL] on FurSPS.ORDER.OrderID = FurSPS.DETAIL.OrderID\r\n"
				+ "where [USER].Type=1 AND FurSPS.ORDER.Status=4\r\n"
